...
{
	int sz = s_packet_size_for_string(param->filename) +
		s_packet_size_for_uint(param->size.low) +
		s_packet_size_for_uint(param->size.high);

	struct s_packet * pkt = s_packet(core->req_id++, S_PKT_TYPE_CREATE, sz);
	if(!pkt) {
		return NULL;
	}
	s_packet_write(pkt, param->filename, string, label_error);
